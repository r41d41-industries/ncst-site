...
/**
 * One-shot migrate for Facebook comments sync table.
 * Run: php sql/apply_facebook_comments.php
 */
...
function facebook_comments_table_exists(PDO $pdo, string $table): bool
{
    $stmt = $pdo->prepare(
...
if (!facebook_comments_table_exists($pdo, $table)) {
    $pdo->exec(
        "CREATE TABLE `{$table}` (
